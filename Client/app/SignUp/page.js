    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-white/10 backdrop-blur-xl rounded-2xl border border-white/30 w-full max-w-md shadow-xl">
        ...
        <div className="px-6 py-4 border-t border-white/20 flex justify-end">
          <button
            onClick={onClose}
            className="px-4 py-2 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white font-bold rounded-xl transition-all transform hover:scale-105 text-sm"
          >
            Got it
          </button>
        </div>
...
export default function SignupPage() {
  ...
  const handleSignup = async () => {
    ...
    try {
      const response = await fetch('https://https://unlimiteddatagh.onrender.com/api/v1/register', {
        method: 'POST',
        headers: {
