...
import { Zap, Star, AlertTriangle, CheckCircle, X, Info, Shield, Phone, CreditCard, ArrowRight, Sparkles, Grid3x3, List, Airplay } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-white/10 backdrop-blur-xl rounded-2xl border border-white/20 w-full max-w-md shadow-xl">
        <div className="bg-gradient-to-r from-yellow-500 via-yellow-400 to-amber-500 px-6 py-4 rounded-t-2xl flex justify-between items-center">
          <h3 className="text-lg font-bold text-gray-900 flex items-center">
            <Zap className="w-5 h-5 mr-2" />
            Purchase {bundle.capacity}GB
          </h3>
          <button onClick={onClose} className="text-gray-900 hover:text-gray-700 p-1 rounded-lg hover:bg-white/20 transition-all">
            <X className="w-4 h-4" />
          </button>
        </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="flex space-x-3">
              ...
              <button
                ...
                className="flex-1 py-3 px-4 bg-gradient-to-r from-yellow-500 to-amber-500 hover:from-yellow-600 hover:to-amber-600 text-gray-900 font-bold rounded-xl transition-all transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
              >
                {isLoading ? (
                  <>
...
const ServiceInfoModal = ({ isOpen, onClose, onConfirm }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-white/10 backdrop-blur-xl rounded-2xl border border-white/20 w-full max-w-md shadow-xl">
        <div className="bg-gradient-to-r from-yellow-500 to-amber-600 px-6 py-4 rounded-t-2xl flex justify-between items-center">
          <h3 className="text-lg font-bold text-gray-900 flex items-center">
            <AlertTriangle className="w-5 h-5 mr-2" />
            Service Notice
          </h3>
          <button onClick={onClose} className="text-gray-900 hover:text-gray-700 p-1 rounded-lg hover:bg-white/10 transition-all">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="px-6 py-4 max-h-[60vh] overflow-y-auto">
          <div className="space-y-3 text-white/80 text-sm">
            <div className="flex items-start">
              <div className="w-1 h-1 rounded-full bg-yellow-400 mr-2 mt-2 flex-shrink-0"></div>
        ...
        <div className="px-6 py-4 border-t border-white/10 flex space-x-3">
          ...
          <button
            onClick={onConfirm}
            className="flex-1 py-2 px-3 bg-gradient-to-r from-yellow-500 to-amber-600 hover:from-yellow-600 hover:to-amber-700 text-gray-900 font-medium rounded-xl transition-all transform hover:scale-105 text-sm"
          >
            Continue
          </button>
        </div>
...
const MTNBundleSelect = () => {
  ...
  const [layout, setLayout] = useState('grid'); // 'grid', 'list', 'carousel'
    ...
    if (layout === 'grid') {
      return (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {bundles.map((bundle) => (
            <BundleCardLarge key={bundle.value} bundle={bundle} onSelect={handleBundleSelect} />
          ))}
        </div>
      );
    } else if (layout === 'list') {
      return (
        <div className="space-y-3">
          {bundles.map((bundle) => (
            <BundleCardList key={bundle.value} bundle={bundle} onSelect={handleBundleSelect} />
          ))}
        </div>
      );
      ...
      return (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          {bundles.map((bundle) => (
            <BundleCardCompact key={bundle.value} bundle={bundle} onSelect={handleBundleSelect} />
          ))}
        </div>
      );
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-yellow-900/20 to-slate-900 relative overflow-hidden">
      {/* Background Elements */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute -top-40 -right-40 w-80 h-80 rounded-full bg-gradient-to-br from-yellow-400/5 to-amber-400/5 blur-3xl animate-pulse"></div>
        <div className="absolute -bottom-40 -left-40 w-80 h-80 rounded-full bg-gradient-to-br from-yellow-400/5 to-amber-400/5 blur-3xl animate-pulse delay-1000"></div>
      </div>
          ...
          <div className="mb-8">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center space-x-3">
                <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-yellow-400 to-amber-500 flex items-center justify-center shadow-lg">
                  <img src="/logos/mtn.png" alt="MTN" className="w-8 h-8 object-contain" />
                </div>
                <div>
                  <h1 className="text-3xl font-bold text-yellow-400">MTN Data Bundles</h1>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className={`p-2 rounded-lg transition-all ${layout === 'grid' ? 'bg-yellow-500 text-gray-900' : 'bg-white/10 text-white hover:bg-white/20'}`}
                  title="Grid View"
                >
                ...
                <button
                  ...
                  className={`p-2 rounded-lg transition-all ${layout === 'list' ? 'bg-yellow-500 text-gray-900' : 'bg-white/10 text-white hover:bg-white/20'}`}
                  title="List View"
                >
                ...
                <button
                  ...
                  className={`p-2 rounded-lg transition-all ${layout === 'carousel' ? 'bg-yellow-500 text-gray-900' : 'bg-white/10 text-white hover:bg-white/20'}`}
                  title="Compact View"
                >
            ...
            <div className="flex justify-center mb-4">
              <button
                ...
                className="flex items-center space-x-2 px-4 py-2 bg-yellow-500/20 border border-yellow-500/50 text-yellow-400 font-medium rounded-lg hover:bg-yellow-500/30 transition-all"
              >
                <Info className="h-4 w-4" />
  ...
  return (
    <button
      ...
      className={`relative p-6 rounded-2xl transition-all border transform hover:scale-105 group ${
        bundle.inStock
          ? 'bg-gradient-to-br from-yellow-400/10 to-amber-500/10 border-yellow-500/30 hover:border-yellow-500/60 cursor-pointer shadow-lg hover:shadow-xl'
          : 'bg-gray-500/10 border-gray-500/20 text-gray-500 cursor-not-allowed opacity-50'
      }`}
    >
      {/* Corner Badge */}
      {bundle.inStock && (
        <div className="absolute top-3 right-3 w-8 h-8 bg-gradient-to-br from-yellow-400 to-amber-500 rounded-full flex items-center justify-center shadow-lg">
      ...
      <div className="text-center">
        <div className="mb-4">
          <div className="inline-block w-16 h-16 bg-gradient-to-br from-yellow-400 to-amber-500 rounded-xl p-1 shadow-md">
            <img src="/logos/mtn.png" alt="MTN" className="w-full h-full object-contain" />
          </div>
        </div>
        
        <h3 className="text-3xl font-bold text-yellow-400 mb-1">{bundle.label}</h3>
        ...
        <div className="p-3 bg-white/10 rounded-lg mb-4">
          ...
          <p className="text-2xl font-bold text-yellow-300">GH₵{bundle.price}</p>
        </div>
        
        {bundle.inStock ? (
          <div className="w-full py-3 px-4 bg-gradient-to-r from-yellow-500 to-amber-500 text-gray-900 font-bold rounded-lg group-hover:shadow-lg transition-all">
            Buy Now
          </div>
        ) : (
          <div className="w-full py-3 px-4 bg-gray-500 text-white font-bold rounded-lg">
            Out of Stock
          </div>
        )}
      </div>
  ...
  return (
    <button
      ...
      className={`w-full p-4 rounded-xl transition-all border flex items-center justify-between ${
        bundle.inStock
          ? 'bg-gradient-to-r from-yellow-400/10 to-amber-500/10 border-yellow-500/30 hover:border-yellow-500/60 hover:bg-yellow-400/20 cursor-pointer'
          : 'bg-gray-500/10 border-gray-500/20 opacity-50'
      }`}
    >
      <div className="flex items-center space-x-4 flex-1">
        <div className="w-12 h-12 bg-gradient-to-br from-yellow-400 to-amber-500 rounded-lg p-1">
          <img src="/logos/mtn.png" alt="MTN" className="w-full h-full object-contain" />
        </div>
        <div className="text-left">
          <h4 className="text-lg font-bold text-yellow-400">{bundle.label}</h4>
          <p className="text-white/60 text-sm">Non-Expiry Bundle</p>
        </div>
      </div>
      <div className="text-right">
        <p className="text-2xl font-bold text-yellow-300">GH₵{bundle.price}</p>
        <div className="w-24 py-2 px-3 bg-gradient-to-r from-yellow-500 to-amber-500 text-gray-900 font-semibold rounded-lg mt-2 text-sm">
          {bundle.inStock ? 'Buy' : 'Out of Stock'}
        </div>
      </div>
    </button>
  );
  ...
  return (
    <button
      ...
      className={`p-3 rounded-lg transition-all border text-center ${
        bundle.inStock
          ? 'bg-gradient-to-br from-yellow-400/20 to-amber-500/20 border-yellow-500/40 hover:border-yellow-500/60 cursor-pointer hover:shadow-lg'
          : 'bg-gray-500/10 border-gray-500/20 opacity-50'
      }`}
    >
      <div className="w-10 h-10 bg-gradient-to-br from-yellow-400 to-amber-500 rounded-lg p-0.5 mx-auto mb-2">
        <img src="/logos/mtn.png" alt="MTN" className="w-full h-full object-contain" />
      </div>
      <p className="text-yellow-400 font-bold text-sm">{bundle.label}</p>
      <p className="text-yellow-300 font-bold text-xs mt-1">GH₵{bundle.price}</p>
    </button>
  );
};
